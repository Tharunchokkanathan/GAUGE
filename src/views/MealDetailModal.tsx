...
import { Flame, Dumbbell, Clock, Sparkles, Droplets, Plus, Check, Utensils, Wheat, Sliders, Calculator } from 'lucide-react';
...
export const MealDetailModal: React.FC<MealDetailModalProps> = ({
  ...
  // State for Dynamic Servings, Oil Grams, and Ingredient Gram Overrides
  ...
  const resolvedIngredients = meal.ingredients.map((ing, idx) => {
    ...
    const per100g = {
      energyKcal: ing.calories / factor,
      proteinG: ing.protein / factor,
      carbohydratesG: (meal.macros.carbs / meal.ingredients.length) / factor,
      fatG: (meal.macros.fat / meal.ingredients.length) / factor,
      fiberG: 3 / factor
    };
  ...
  const handleAdd = () => {
    if (onAddMeal) {
      ...
      onAddMeal(meal, servings, mapOilLevel);
      setIsAdded(true);
      setTimeout(() => {
        setIsAdded(false);
        onClose();
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={meal.name} maxWidth="lg">
      <div className="space-y-6 pb-2">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
              <Calculator className="w-3.5 h-3.5 text-emerald-400" /> Dynamic Calculated Nutrition
            </h4>
            <span className="text-[11px] text-emerald-400 font-mono">Real-time Mifflin-St Jeor Engine</span>
          </div>
              ...
              <div key={idx} className="py-2.5 flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 bg-slate-950/80 border border-slate-800 rounded-lg p-1">
                  <button
                    ...
                    className="w-6 h-6 rounded bg-slate-800 text-slate-300 hover:bg-slate-700 flex items-center justify-center font-mono font-bold"
                  >
                    -
                  </button>
        ...
        {/* Explicit Math & Calculation Formula Box */}
        <div className="p-4 rounded-xl bg-slate-900/90 border border-slate-800 space-y-2 text-xs font-mono">
          <div className="flex items-center justify-between text-slate-400 border-b border-slate-800 pb-2">
            <span className="font-sans font-bold text-white flex items-center gap-1.5">
              <Calculator className="w-4 h-4 text-emerald-400" /> Transparent Calculation Formula
            </span>
            <span className="text-[10px] text-emerald-400">Deterministic Engine</span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-slate-300">
            <div>Base Ingredients Total: <span className="text-amber-400 font-bold">{Math.round(rawBaseCalories)} kcal</span></div>
            <div>Oil Addition ({oilGrams}g): <span className="text-amber-400 font-bold">+{oilCalories} kcal</span></div>
            <div>Combined Batch Energy: <span className="text-emerald-400 font-bold">{Math.round(batchTotals.energyKcal)} kcal</span></div>
            <div>Serving Scaled ({servings}x): <span className="text-emerald-400 font-extrabold">{finalCalories} kcal</span></div>
          </div>
        </div>
        ...
        <div className="pt-4 border-t border-slate-800 flex items-center gap-3">
          <Button
            ...
            onClick={handleAdd}
          >
            {isAdded ? 'Added to Today’s Plan!' : `Add to Plan (${finalCalories} kcal • ${finalProtein}g protein)`}
          </Button>
        </div>
      </div>
    </Modal>
